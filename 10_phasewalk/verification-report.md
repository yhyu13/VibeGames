...
## v4.26 打磨轮 25（2026-08-16）✅ — 对抗验证 → pad3 glyph 塔柱遮挡修正

> round-24 把 pad3 西移至 (0.3,1.5) 修好了固化半径失察，却引入了新回归：中心径向 1.53m 虽 >1.4（清塔柱），但 0.72m glyph 的南缘坐于 z=1.14，落在不透明中央塔柱内（塔柱最北 z=1.4）。21 代理对抗验证（`wyk99v9jx`）确认 7 项、refute 6 项；落 3 处代码 + 2 处 doc：

**① pad3 glyph ~29% 被塔柱遮挡（levels.ts，medium）**：pad3 (0.3,1.5) 的 glyph（0.72² 平面）南缘 z=1.14 深入塔柱 footprint（x²+z²<1.96，最北 z=1.4）——中心虽在塔外，南半 glyph 却落在塔柱内。z=1.5 处无任何 pad3 x 位置能同时 glyph 清塔（需 |x|>1.17）、池清固化（需 x<0.48）、左到右（需 x>-0.8）三条件共存，故整行北移：pad1–3 z 1.5→1.9，glyph 南缘 z=1.54 完全清出塔柱。
