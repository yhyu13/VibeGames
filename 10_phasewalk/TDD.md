...
**Level rules**: 5 layers × 5m；每层 ≤ 24 platforms（**每相 ≤ 8**；F1 启示厅四相各一条路线汇聚金门、F2–F4 单相为主、F5 四相均衡）；每层 4 相尘（每相路线 1 枚）；出口金门 = 收集该层 ≥3/4 相尘打开（**探索驱动：必须掌握 ≥3 相**）。**死亡政策（2026-08-14 playtest）**：危险 = 无相区（`Hazard phases='all'`，无相者吃相）、引流管（`Pipe.danger` 陷阱，吸入虚空）、雷云（气相专属）、虚空（y < −2）；死亡 → **出生点重生 + 相位重置固 + deaths 计数**，绝无同点重试，相尘保留。教学节奏（世界观先行，`docs/design/00-worldview-first.md`）：F1 前 5 分钟每拍 ≤60s 揭示一个新真相，F2–F4 教学相平台量 ≥ 50%，F5 四相均衡。可达性法则：任意相尘/出口 ≤ 该相 2 连跳可达（水平 ≤6.5m、垂直 ≤3.4m）。
