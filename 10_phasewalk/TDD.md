# TDD — PHASEWALK (四相行者) (current contract v0.16)
...
| v0.16 | 2026-08-16 | 打磨轮 24：回归验证确认 6 项（均源于 pad3 在固化半径内）——①F1 `pad3` 从 (0.8,1.5) 西移至 (0.3,1.5)：旧位置距 pool1 中心 1.29m < `SOLIDIFY_RADIUS` 1.6，固相踩 pad3 即自动凝池、永久压制凝桥提示；新位置 1.78m（出半径），仍清塔柱（径向 1.53>1.4）与无相区（x<1.5）；②HUD `poolNear` 去掉 `!pool.solidified` 门控 + 三态文案（未凝「走近相液池…凝成桥」/已凝「相液已成桥…跨过无相区」）——池若在 pad3→pad4 途中被凝，跨桥教学不再退化为通用「空格 跳」；③HUD 注释更正 pad 距池实测（pad1 4.56/pad2 2.87/pad3 1.78/pad4 1.80，非旧「1.7–2.9」）；④`stepPassword` 注释更正 pad3→pad4 对角 2.56m/死缝 0.76m、相邻踩踏圈重叠随 1.1–1.7m 间距 |
