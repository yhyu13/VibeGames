# TDD — PHASEWALK (四相行者) (current contract v0.18)
...
| v0.13 | 2026-08-15 | 打磨轮 21：对抗验证（54 代理 / 6 lens / 3 票 refute）确认 7 项、落 6 处——`stepPassword` latch 改仅**水平离开**清除（垂直上限只门控新踩踏，同板原地跳不再越过 STEP_HEIGHT 清 latch 后重判为新步踩错重置）；轨道拖拽按 `pointerId` 跟踪 + `pointerup` 门控主键（右键/中键释放、触摸第二触点 cancel 不再中断左拖）；径向菜单去 `onMouseLeave` 清空（悬停不再清空键盘选择，`hoverPhase(null)` 变 no-op，last-input-wins）；`.radial` 改回 `pointer-events:none`（四象限仍 auto，菜单开启时轨道镜头仍可拖观察）；HUD 首拍 Tab 教学上移为最高优先级（新玩家先学切相再学踩序）；F1 `pad4` 再南移 z 0.7→0.2（踩踏圈北缘 1.1<1.2 完全清出无相区） |
| v0.14 | 2026-08-16 | 打磨轮 22：回归验证确认 7 项、落 5 处——`stepPassword` latch 改「**踩离踩踏圈**才清」（不再「最近石板变了就清」：相邻石板踩踏圈重叠 0.1–0.2m，最近石板在中垂线翻转，骑缝/原地横向微抖会清 latch 后把同一步重判为新步踩错重置；现踩着同板水平半径内即 hold，真踩离才 re-arm）；`onPointerDown` 加 `dragging || !e.isPrimary` 门控（第二触点既不 hijack 也不启动轨道拖拽）；径向菜单改**双字段模型**（`highlighted`=键盘提交 / `hovered`=悬停暂态，显示+释放取 `hovered ?? highlighted`，悬停离开回退键盘而非永久 latch——orbit 拖拽扫过象限不再误切相）；HUD 首拍 Tab 教学加 `passwordProgress===0` 门控（已入谜者看到「密文 x/4」而非切相教学）；HUD 凝池「固化造路」上移到密文提示之前（无相区=即死，凝桥教学不再被密文挤掉） |
| v0.15 | 2026-08-16 | 打磨轮 23：回归验证（54 代理 / 6 lens / 3 票 refute）确认 7 项、落 4 处 + 1 接受——①F1 密文石板符号改**左到右与踩序一致**（pad1=石 pad2=流 pad3=息 pad4=焰；旧乱序 pad1=流 pad2=石 pad3=焰 pad4=息 迫使 pad2→pad1→pad4 直线穿过 pad2/pad3 踩踏圈重置进度、第 3 符号永不入账，须绕行 z<0.6；现单向西→东，hide-and-seek 仍由透明相玻板承载）；②`RadialMenu` 悬停加 `e.buttons===0` 门控（button 按下的轨道拖拽扫过/停在象限不再 latch `hovered` 误切相——round-22 双字段只修了 sweep 未修 rest-at-release）；③HUD 凝池提示加 `!onPad` 门控（水平半径 0.9 与 `stepPassword` 同语义；半径 3.2 的 poolNear 吞掉 pad2/pad4 密文教学且距池 > 固化半径 1.6 永不自清）；④`stepPassword` 注释更正 pad3→pad4 为 0.34m 死缝非重叠；⑤（接受）菜单开启瞬间光标已停象限不高亮——需移动 1px，cosmetic 不改 |
| v0.16 | 2026-08-16 | 打磨轮 24：回归验证确认 6 项（均源于 pad3 在固化半径内）——①F1 `pad3` 从 (0.8,1.5) 西移至 (0.3,1.5)：旧位置距 pool1 中心 1.29m < `SOLIDIFY_RADIUS` 1.6，固相踩 pad3 即自动凝池、永久压制凝桥提示；新位置 1.78m（出半径），仍清塔柱（径向 1.53>1.4）与无相区（x<1.5）；②HUD `poolNear` 去掉 `!pool.solidified` 门控 + 三态文案（未凝「走近相液池…凝成桥」/已凝「相液已成桥…跨过无相区」）——池若在 pad3→pad4 途中被凝，跨桥教学不再退化为通用「空格 跳」；③HUD 注释更正 pad 距池实测（pad1 4.56/pad2 2.87/pad3 1.78/pad4 1.66，非旧「1.7–2.9」）；④`stepPassword` 注释更正 pad3→pad4 对角 2.56m/死缝 0.76m、相邻踩踏圈重叠随 1.1–1.7m 间距 |
| v0.17 | 2026-08-16 | 打磨轮 25：回归验证确认 7 项、落 3 处——①F1 石板行 pad1–3 从 z=1.5 北移至 z=1.9（round-24 的 (0.3,1.5) 虽径向 1.53 清出中央塔柱，但 0.72m glyph 南缘坐于 z=1.14 落在塔柱内（塔柱最北 z=1.4），~29% glyph 被不透明塔柱遮挡；z=1.9 使 glyph 南缘 z=1.54 完全清出）；②HUD/levels 注释更正 pad 距池实测（pad1 4.55/pad2 2.85/pad3 1.75/pad4 1.66）与「pad3→pad4 仍擦进固化半径（~0.99m，三元提示兜底）」；③`SceneManager` 相玻板 z 注释/兜底 1.5→1.9 |
| v0.18 | 2026-08-16 | 打磨轮 26：回归验证（`w4fn0br53`）复核 walk 距离——~1.15m 为 round-24（pad3 z=1.5）陈旧值，z=1.9 后 pad3→pad4 对角到池中心 (2.05,1.8) 最近 ~0.99m（t≈0.52，√0.982）；更正 levels/HUD/TDD/verification 四处 + 澄清 dy = 玩家站立 y=0.6 − 池 y=0.75 = −0.15（`solidifyFluids` 用玩家位置非 pad 瓷砖 y=0.05，3D 只加 ~0.01m） |
...
**密文石板（v4.19 透明板 hide-and-seek）**：`LayerData.password`（正确踩序 `PhaseId[]`）+ `LayerData.passwordPads`（地板瓷砖，各刻一相 glyph）。玩家按透明相玻板（`SceneManager` 低不透明度 CanvasTexture 板，悬浮于瓷砖上方）上的顺序踩瓷砖；`stepPassword` 边沿步进（`passwordPadId` 防站立重复触发）：踩对下一符号 `passwordProgress++`，踩错归零。`gateOpen()` 在有密文的层额外要求 `passwordProgress ≥ password.length`——密文 + ≥3 相尘 + 无存活守层者三者齐才开门。死亡不回退密文（`passwordProgress` 跨死亡保留，`respawnAtSpawn` 不碰它；仅错误步进归零）。F1 启示厅密文 = [石→流→息→焰]，四瓷砖排在出生台前（近横排：pad1–3 一行 z=1.9，pad4 南移 z=0.2 清出无相区），答案刻在悬浮的透明相玻板上（轨道镜头 Q/E/拖拽可对正读取）。**v4.20 修四处**：(1) `stepPassword` 取**最近**石板（重叠踩踏圈取几何最近，非数组首项）；踩错归零时若踩到的是**首符号**则直接记 `progress=1`（正确重开无需踩离再踩回）；解谜后 `passwordPadId` 清 null（脚下 latch 不再残留）；(2) HUD 密文 pip 只显**已踩对**的符号（未来符号渲染为中性点，答案不被 HUD 泄露）；(3) 提示锚**最近**石板（非 pad1，中/右石板入场也能看到教学）；(4) F1 `pad4`（息，第 3 步）从 (2.5,1.5) 北移至 (2.5,0.7)——原位置在无相区 hA 内（踩上即死）且被凝池桥遮挡。**v4.21 补一处**：`stepPassword` 加垂直上限 `STEP_HEIGHT=1.6`——仅近地（站立/小跳）判定为踩踏，高空跳跃/气飘越过石板行不再误触发踩序重置（pad4 移出凝池桥后，原「气飘踩 pad4 需不限高」的意图已消失，水平不限高的理由随之取消）。**v4.22 补两处**：(1) `stepPassword` 的边沿 latch 改仅**水平离开**清除——垂直上限 `STEP_HEIGHT` 只门控**新**踩踏，不再在最近石板搜索里 `continue` 跳过头顶石板：原先同一石板上原地跳（中心越过 1.6 → `onPad` 清 null → `passwordPadId` 清 → 落地重判为新步）会把已踩对的 progress 当新步踩错重置，或气/液相在 1.6 边界上下浮动时每帧重 latch；现在空中掠过未 latch 的另一块石板仍不 latch（垂直上限在「新 latch」后生效），但原地跳不清 latch。(2) F1 `pad4`（息，第 3 步）从 (2.5,0.7) 再南移至 (2.5,0.2)——0.7 距无相区 hA 的 z-min 1.2 仅 0.5m，踩踏圈（半径 0.9）北缘 z=1.6 仍探入即死区，玩家踩北半侧即死；0.2 使踩踏圈北缘 z=1.1<1.2 完全清出。**v4.23 补一处**：`stepPassword` 的 latch 从「最近石板变了即清」改为「**踩离该石板踩踏圈**才清」（水平距离 > 半径 0.9 才 re-arm）——v4.22 的「水平离开清 latch」实为「最近石板 flip 即清」，而相邻石板踩踏圈重叠 0.1–0.2m（间距 1.6–1.7 vs 半径 0.9），最近石板在中垂线两侧翻转，玩家骑缝或原地横向微抖（±0.05m）会清 latch 后把同一步重判为新步踩错重置；现踩着同板水平半径内即 hold（原地跳/骑缝都不重触发），真踩离才进入新最近石板判定。**v4.24 补三处**：(1) F1 密文石板符号改**左到右与踩序一致**（pad1=石 pad2=流 pad3=息 pad4=焰）——旧乱序（pad1=流 pad2=石 pad3=焰 pad4=息）迫使玩家在石板行间折返（pad2→pad1→pad4 直线穿过 pad2/pad3 踩踏圈重置进度、第 3 符号永不入账，须绕行 z<0.6）；线性顺序让踩踏路径 = 单向西→东，hide-and-seek 仍由透明相玻板承载（顺序依旧隐藏）。(2) `stepPassword` 注释更正 pad3→pad4 对角 2.14m 为 0.34m 死缝（非重叠）。(3) HUD 凝池提示加 `!onPad` 门控（水平半径 0.9，与 `stepPassword` 同语义）——半径 3.2 的 poolNear 会吞掉 pad2/pad3/pad4 的密文教学（第 2/3/末步），且 pad2/pad4 距池 > 固化半径 1.6 永不自清；现在只在真正走到无相区入口（离开石板）时才提示凝桥，并靠固化自清。**v4.25 补两处（round 24）**：(1) F1 `pad3`（息，第 3 步）从 (0.8,1.5) 西移至 (0.3,1.5)——旧位距池 1 中心 (2.05,1.8) 仅 1.29m，落在 `SOLIDIFY_RADIUS` 1.6 内：固相踩 pad3 会当场把相液池冻成桥（v4.24 的 `!onPad` 门控只挡「凝池提示」，挡不住 `solidifyFluids` 本体，桥一成型「走近相液池 · 凝成桥」教学就被 `!pool.solidified` 永久压掉）；西移后距池中心 1.78m（>1.6），径向 1.53m 仍清出中央塔柱（半径 1.4）与东侧无相区（x<1.5），pad1–3 仍沿 z=1.5 一行、pad4 南移 z=0.2 的 L 形不变。(2) HUD `poolNear` 移除 `!pool.solidified`、凝池提示改三元（桥成 →「相液已成桥 · 石相跨过无相区」/ 未成 →「走近相液池 · 石相会把它凝成桥」）——即便 pad3→pad4 最后一段对角仍擦进固化半径（~1.345m，池会在此段提前冻结），跨桥教学也不再退化为通用动词提示。**v4.26 补两处（round 25）**：(1) F1 石板行 pad1–3 从 z=1.5 北移至 z=1.9——round-24 的 (0.3,1.5) 虽径向 1.53 清出中央塔柱（半径 1.4），但 0.72m glyph 的南缘坐于 z=1.14，落在塔柱内（塔柱最北 z=1.4），~29% glyph 被不透明塔柱遮挡；z=1.9 使 glyph 南缘 z=1.54 完全清出塔柱，且 pad 中心仍出固化半径（pad1 4.55 / pad2 2.85 / pad3 1.75 / pad4 1.66 距 pool1 中心）。pad3→pad4 最后一段对角仍擦进固化半径（~0.99m，池在此段提前冻结），round-24 的三元提示已兜底。(2) HUD/levels 注释更正 pad4 距池实测 1.80→1.66（旧 1.80 误用 pad 瓷砖 y=0.05，非玩家站立 y=0.6）。
