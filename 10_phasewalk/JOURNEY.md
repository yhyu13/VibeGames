...
| C5 | **四种物质动词** | 固=固化造路(冻相液池成桥) / 液=分离 / 气=穿过(穿弹穿栅) / 焰=吸收反弹 | ✅ |
...
| A2 | 数据驱动 | `sfx.ts` 9 条 recipe（波形/起止频/时长/音量），`AudioManager` 薄合成器，零硬编码音频 | ✅ |
...
| U2 | HUD 教学阶梯 | 状态机式 hint 阶梯（10 级优先链）教四个动词 + 凝桥 + 子弹，前 45s HUD 隐藏 | ✅ |
...
| # | 特性 | 一句话 | 状态 |
|---|---|---|---|
| L1 | 中央塔 14×14m | 金门在塔顶，四条相路从四侧爬同一座塔汇聚（固=西面石阶 / 液=竖直水柱 / 气=开放缺口 / 焰=爆冲台+反射） | ✅ |
...
- **一个四动词塔爬升器**：四个相是四种**独立（垂直）又互补**的移动/物质动词——每个相回答一个不同问题（造路 / 穿缝 / 绕行 / 战斗），不是「怎么往上走」×4。
...
- **不是「往上走 ×4」**：四相不是四种移动方式的重皮，而是四个互补的问题（造路/穿缝/绕行/战斗）。
...
## 附录 B. 已知 doc drift（本次评审要看到的诚实清单）

> 这些是**当前已知的文档不一致**，不是 bug，但下次改动前要先修：

1. `AGENTS.md` 落后 v4 一个版本：仍列 `traverse.ts`，F1 路线仍写 v3 的「南石阶/西流槽/北风井/东电线」。
2. `TDD.md` §7 分支策略仍写「phasePhysics+traverse」文件清单（traverse.ts 已删除）。
3. `docs/review.md` 描述的是 v0.1（无敌人）；v4 已解冻「无敌人」引入相灵眼发射器/子弹。
4. `src/components/LayerClear.tsx` 不存在——层结算在 App.tsx 循环内用 audio+粒子处理，无独立 React 覆盖层（TDD §2 树里仍列了它）。

> 结论：**核心代码与 v4 设计完全一致**（tsc 0 error、build green、17+6 playtest 全过），drift 集中在「讲述 v3 旧事的文档」上——这正是 JOURNEY.md 要补的那张全景图。
