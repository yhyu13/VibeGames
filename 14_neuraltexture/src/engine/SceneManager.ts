...
import { createGLSLNeuralMaterial } from './fallbackMaterial'
import { createNeuralMaterial } from './NeuralMaterial'
...
// Common surface both backend materials expose: the mesh material plus a movable
// key-light angle. Each backend supplies its own impl; the rest of the scene is shared.
interface NeuralScene {
  material: THREE.Material
  setLightAngle: (a: number) => void
}

// One renderer-agnostic scene builder. The WebGPU and WebGL2 fallback differ only in
// renderer class + material factory; everything else is identical, so it lives here
// once instead of being duplicated across the two public entry points.
function buildScene(
  host: HTMLElement,
  renderer: THREE.WebGLRenderer | WebGPURenderer,
  makeNeural: () => NeuralScene,
  mode: RendererMode,
): SceneHandle {
...
/**
 * WebGPU path: the WGSL neural material demo. GPU-less browsers fall through to
 * createSceneWebGL, which reuses the same scene via buildScene.
 */
export async function createScene(host: HTMLElement): Promise<SceneHandle> {
  const renderer = new WebGPURenderer({ antialias: true })
  await renderer.init()
  return buildScene(host, renderer, createNeuralMaterial, 'webgpu')
}

/**
 * WebGL2 fallback scene. GPU-less browsers still get the three-identical-material
 * teacher / neural / error trio via a GLSL re-implementation of the WGSL shader,
 * instead of the previous hard "WebGPU NOT available" error string.
 */
export async function createSceneWebGL(host: HTMLElement): Promise<SceneHandle> {
  const renderer = new THREE.WebGLRenderer({ antialias: true })
  return buildScene(host, renderer, createGLSLNeuralMaterial, 'webgl2')
}
