...
| 文件 | 说明 |
|---|---|
| `docs/how-to-play.md` | v2 玩法文档（markdown 源） |
...
4. v2 截图还需覆盖屏幕相对 WASD、鼠标短谱、中央安全区和五类弹幕表现；具体时机随新版实现更新。
...
- **v1 一页版** `v1/how-to-play.html`：归档的暗色主题页面，图片路径相对于 `docs/v1/`。
- **v1 PPT 版** `v1/how-to-play-slides.html`：归档幻灯片，键盘翻页。
- **v2 Markdown** `how-to-play.md`：先作为新版玩法的唯一文字事实源；新版 HTML / 幻灯片待 v2 截图完成后再生成。
- 两个 HTML 都是单文件自包含（无外部依赖），直接用浏览器打开 `file:///` 即可。

## 4. 验证

```bash
# Playwright 检查 v1 归档 HTML 图片全部加载（broken=0）
