...
// 纯 beat 模型：推进 stage/beat 索引、站位容差（A1）、打断检测。
// 攻击节拍判定由 attackJudgement.ts 在 LMB 按下时完成。
...
export function advanceBeat(
  ...
  if (!script || script.stages.length === 0 || stageIndex < 0 || stageIndex >= script.stages.length) {
    return { stageIndex: Math.max(0, stageIndex), beatIndex: 0, done: true, broke: false };
  }
  let sIdx = stageIndex;
  let bIdx = beatIndex;
  ...
  if (!stage || stage.beats.length === 0) {
    const next = { ...advanceBeat(script, sIdx + 1, 0, 0, interrupts) };
  ...
  if (beat.type === 'line') {
    ...
    if (hitBreak || barrageBreak || forgetBreak) {
      ...
      return { stageIndex: sIdx, beatIndex: bIdx, done: false, broke: true };
    }
  }

  // beat 未完成 → 原地等待
  if (elapsed < beat.duration) {
    return { stageIndex: sIdx, beatIndex: bIdx, done: false, broke: false };
  }
  ...
  if (sIdx >= script.stages.length) {
    ...
    return { stageIndex: sIdx, beatIndex: Math.max(0, stage.beats.length - 1), done: true, broke: false };
  }
  return { stageIndex: sIdx, beatIndex: bIdx, done: false, broke: false };
}
