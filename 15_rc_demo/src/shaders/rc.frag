...
uniform int   uMergeMode; // 0=demo 镜像映射 1=本像素双线性 2=射线终点双线性
uniform int   uCanonicalSpacing; // 1=canonical(cascade0 最细,越高越粗) 0=demo(相反)
...
probe get_probe_info(int index) {
  ...
  float probeAmount = pow(
    float(uBaseRayCount),
    uCanonicalSpacing == 1 ? float(uCascadeAmount - index) : float(index)
  );
...
vec4 radiance_interval(vec2 uv, vec2 dir, float a, float b, out vec2 endUv) {
  uv += a * dir;
...
void main() {
  ...
  for (float i = 0.0; i < float(uBaseRayCount); i += 1.0) {
    ...
    if (!(LAST_LEVEL) && deltaRadiance.a == 0.0 && uDisableMerging != 1) {
      if (uMergeMode == 1) {
        // canonical：本像素位置双线性采样上一 pass（cascade 纹理 LINEAR）
        deltaRadiance += texture(uLastPass, gl_FragCoord.xy / uResolution);
      } else if (uMergeMode == 2) {
        // 射线终点：上一 pass 在射线走完本区间后的位置接管
        deltaRadiance += texture(uLastPass, clamp(rayEnd, 0.0, 1.0));
      } else {
        // demo 原版：按射线方向映射到上一 pass 的 probe 单元
        up.position = vec2(
