...
export function FinanceDynastyChoice() {
  ...
  const relationshipTrust = useGameStore((s) => s.state.relationshipTrust)
      ...
      <span>
        {origin !== 'finance_dynasty'
          ...
          : relationshipResolved
            ? `你继承了资源,也学会了把关系当关系 —— 关系信任 ${Math.round(relationshipTrust)}/100。`
            : relationshipClosed
              ? `你守住了资产,却让这段关系停在门外(信任 ${Math.round(relationshipTrust)}/100)。`
              : `资源能打开门,却不能替你建立信任(信任 ${Math.round(relationshipTrust)}/100)。`}
      </span>
      {origin !== 'finance_dynasty' && (
