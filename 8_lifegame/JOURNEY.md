...
- **完整 backlog 见 `docs/roadmap-100.md`**（100 任务 P0/P1/P2 分档，2026-08-31 起）—— 本节只列最高优先级的几个。
- **穿越AI（Ch10，下一个）** —— 用户提的「知道未来的 AI 指导做题家投资」。朴素版会拆掉「认知觉醒靠挣 + 信息差承重墙」；能救活的版本 = 穿越AI × 接住质量（宏观未来提示，认知不够就接不住，复用 Ch07 机制）。roadmap P1。
- **付费贵人 / 贵人流转 / Ch09 金牌策略** —— 出范围，留 M2+（缺 Token 系统 / 多时代）。roadmap P2。
- **重探针（smoke-seeds/seeds10/marathon）在本环境 headless 超时** —— rAF 降频，非代码问题；本环境用 `scripts/quick-gate.mjs`（纯 sim 快速门），重探针留作有头环境（roadmap P0 #3 已落地）。
- **Ch07 收紧了胜利条件** —— 破契约：任何 mentor_hit 都是胜利 → 仅信任命中（认知≥60+AI）才是大觉醒（Ch07 era）。
...
## 9. Ch09 投资策略库 + 真实度自选 —— v3.1（2026-08-31）

| 我 | 你 |
|---|---|
| 先问「现在投资能否一轮多品种 / 手续费设定 / 和真实模拟盘差距」 | 诚实盘点：委托篮已支持多品种、佣金万三一刀切、T+1（BTC 除外）；差距集中在分品种费率 + 日内/订单/分红（后者有意不接） |
| 拍板「要好玩，简单」 | 把「好玩」定为「加选择不加复杂」—— 值得加的是选择点（策略/真实度自选），不是保真（日内/止损单/做空） |
| 提「模拟盘可设新手/高手/真实，玩家自选真实度」 | 认可这是比「一套规则强加所有人」更好的直觉，但收窄成 2 档（新手/真实，测试面 ×2 非 ×3） |
| 提「穿越AI 指导做题家投资（全新 idea，你可以 shoot down）」 | **部分 shoot down**：知道未来的 AI 会拆掉「认知觉醒靠挣 + 信息差是承重墙」这面命根子墙；给出能救活的版本 = 穿越AI × 接住质量（认知不够就接不住未来提示，复用 Ch07 机制），拆去 Ch10 |
| 确认 Ch09 范围 = 真实度自选 + 策略层 + 分品种费率 | 写 spec `design/21` + red-first 探针 `strategy-probe.mjs`；critic 硬化抓 4 blocking（假信号公式矛盾 / 平仓接缝冲突 / realism 接缝 / 探针静默跳过）全修 |
| 「按 dev-loop 开工建 Ch09」 | 实现 3 机制 + InvestPanel UI（真实度开关 / 持有-择时分段 / 属性卡费率）；critic #2（实现）0 blocking + 7 nit，修了 4 个（取整护栏 / 新手档残留择时徽标 / 探针脆弱 / 探针静默） |
| —— | 全门绿：tsc 0 + build + strategy-probe + showcase 小镇/dynasty 回放 + basket + interaction + keyboard + layout + contrast，0 console error |

---

## 10. roadmap-100 + P0 清扫 —— 全景规划（2026-08-31）

| 我 | 你 |
|---|---|
| 要「plan 100 task from p0 to p2 dump to doc」 | 派 code audit 子代理全量读 src/ + 文档 + 探针（结论：0 个 crash 级 bug,5 个 P1 错误行为 + 15 个 P2 polish/死代码）；起草 `docs/roadmap-100.md`(100 任务按 P0/P1/P2 分 15 子系统) |
| 要「review plan doc」 | 派 fresh-context critic 审 plan doc —— 抓 6 个 blocking（文档漂移漏了 4 处 / P0 与 P1 任务重复 / 占位符不可执行 / polish 误分类进 P0 / verification-report 头部漂移漏掉 / 30% 上限锚点未验证），全修 |
| 要「only do P0, finish them all」 | 定稿 P0 = 7 项（正确性/可靠性/文档一致性/误导玩家），全部做完：图层溢出（scale 1.08→1.02 精确补偿）+ 文档漂移 5+ 处 + quick-gate 快速验证门（headless 超时对策）+ 新手档 T+1 提示撒谎 + 卖出残留择时徽标 + 2 个探针断言 |
| —— | 审计最有价值的发现是「没有 P0 crash」—— 代码在正确性上是干净的，剩下的都是「玩家可见缺陷/文档漂移/误导玩家」这一档 |
