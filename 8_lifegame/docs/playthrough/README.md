...
## 1. 第 0 步 · 出身定型(第一局固定小镇做题家)

![选择出身](01-origin-choice.png)

**第一局出身是固定的「小镇做题家」**(原生叙事,没有选择器)。通关(大觉醒)后才解锁**金融世家**重开 —— 两条路线差异很大,但**玩法机制完全一样**(只是初始数值、骰子修正、模拟盘起点不同):
...
- 骰点(2d6,2–12)+ 出身修正(小镇 −2 / 世家 +2)+ 状态修正 = 最终档位:大失败(2–3)/ 失败(4–6)/ 成功(7–9)/ 大成功(10–12)/ 高光时刻(13+)。
...
- **佣金**:分品种(真实档:货币基金 0.01% / 指数基金 0.05% / 黄金 0.02% / A股 万三 0.03% / 港股 0.05% / BTC 0.1%),买卖双向都收;**新手档免佣**。
...
| 3 | 教学楼 | **选方向 → 人工智能** |
| 4–8 | 图书馆 / 教学楼 | 堆认知到 ≥ 60,解锁健身房与对外交流中心 |
| 9 | 健身房 | **办卡**(首次进入触发),解锁身心回复点 |
| 10–13 | 对外交流 + 贵人办公室 | 认知冲 90+,同时试贵人认可 |
| 14–16 | 寒假事件(自动) | 收尾感情线 |
...
- 版本:**v2.13**(任天堂式交互手感)及之前已合入的所有机制(v1.2 认知信息、v1.3 开户、v1.6 选方向+贵人信任、v1.7 办卡+对外交流、v2.4 模拟盘、v2.5 爱情+人生目标、v2.6 贫困逻辑、v2.8 渐进解锁、v2.11 委托篮)。
- 截图来源:确定性回放脚本 `scripts/showcase.mjs`(小镇路线),种子固定,0 console error。
- 相关设计文档:`docs/design/` 下 01–19 号(尤其 12-v2.7 规则/换向、17-v2.11 委托篮、18-v2.12 两步走、19-v2.13 任天堂手感;14/15/16 号全天打磨计划已归档 `old/`)。
