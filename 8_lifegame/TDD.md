...
**人生目标 (v2.5)**: the opening cinematic's second card establishes the goals. Wealth goal per origin — 小镇 ¥150,000 / 世家 ¥400,000 (≈ starting wealth +50%/+33%) — lives in `GameState.lifeGoalWealth`, tracked as a HUD progress chip and a summary verdict (达成 when `player.wealth ≥ lifeGoalWealth`). Progress reads NET of the origin's starting wealth (`lifeGoalProgressFor` — town ¥100k → ¥50k to earn; dynasty ¥300k → ¥100k to earn), so a run starts at 0%, not an inherited 67%. The love goal is stage-derived (达成 when `loveStage === 'close' || loveReunion`). Goals are read-only presentation state; they never change any probability, delta, or unlock.
