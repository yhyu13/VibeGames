...
export class SceneManager {
  ...
  render(s: SimSnapshot, dt: number): void {
    ...
    if (distanceBetween(s.player.position, lamp.position) <= 2.45 && lamp.state !== 'dead') { c.fillStyle = '#ffd06a'; c.font = `${Math.max(9, scale * .18)}px monospace`; c.textAlign = 'center'; c.fillText(lamp.state === 'damaged' ? '再击一次 · LMB' : '已进入攻击范围 · LMB ×2', lamp.position.x * scale, lamp.position.y * scale - scale * .7); }
  ...
  private drawFlashlightCone(c: CanvasRenderingContext2D, p: Vec2, angle: number, z: number): void { c.save(); c.translate(p.x*z,p.y*z); c.rotate(angle); c.fillStyle='rgba(242,216,146,.08)'; c.beginPath(); c.moveTo(z*.2,0); c.arc(0,0,z*4.2,-20*Math.PI/180,20*Math.PI/180); c.closePath(); c.fill(); c.strokeStyle='rgba(248,221,157,.14)'; c.lineWidth=1; c.beginPath(); c.arc(0,0,z*4.2,-20*Math.PI/180,20*Math.PI/180); c.stroke(); c.restore(); }
}
