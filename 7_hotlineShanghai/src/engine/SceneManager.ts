...
export class SceneManager {
  ...
  private drawFlashlightCone(c: CanvasRenderingContext2D, p: Vec2, angle: number, z: number): void { c.save(); c.translate(p.x*z,p.y*z); c.rotate(angle); c.fillStyle='rgba(255,244,205,.18)'; c.beginPath(); c.moveTo(0,0); c.arc(0,0,z*8,-25*Math.PI/180,25*Math.PI/180); c.closePath(); c.fill(); c.strokeStyle='rgba(255,244,205,.26)'; c.lineWidth=1; c.beginPath(); c.arc(0,0,z*8,-25*Math.PI/180,25*Math.PI/180); c.stroke(); c.fillStyle='rgba(255,177,55,.46)'; c.beginPath(); c.moveTo(0,0); c.arc(0,0,z*2.5,-25*Math.PI/180,25*Math.PI/180); c.closePath(); c.fill(); c.strokeStyle='rgba(255,205,95,.8)'; c.lineWidth=2; c.beginPath(); c.arc(0,0,z*2.5,-25*Math.PI/180,25*Math.PI/180); c.stroke(); c.restore(); }
}
