...
description: Scope a creative project to ONE scene and polish it until perfect. 5-phase standard process (audit → critic → enumerate → plan → execute) + intro-scene-until-perfect (single-scene → vertical slice → polish → infinite loop). Triggers: '只需要 1 个 X' / '只 ship intro scene' / '全力打磨直到完美' / '给一个 plan doc' / '可以 X' (解冻 forbidden list). Produces 3 件套人类可读 doc (设计 GDD-level / 美术 Art Book / 代码 Code Book).
metadata:
  type: skill
  origin: 7_hotlineShanghai (2026-08-09 commit a80503c)
  case-study: 7_hotlineShanghai
---

# intro-scene-until-perfect (Codex skill)

> **用法(Codex CLI)**:
> - 项目根的 `AGENTS.md` 已存在(7_hotlineShanghai 已有)。本文件 `.codex/skills/intro-scene-until-perfect.md` 是 modular skill,**在 Codex CLI 中需要通过 `@.codex/skills/intro-scene-until-perfect.md` 在 prompt 中引用**,或在 `AGENTS.md` 中 link。
> - 如果 Codex CLI 不支持 modular skills,可以把本文件的 body 复制到 `AGENTS.md` 末尾作为参考章节。
>
> **用法(Codex App, 2026-02+)**:
> - Codex App 原生支持 skills 概念。本文件位于 `.codex/skills/`,App 应自动加载。
>
> **来源**:这是 `.claude/skills/intro-scene-until-perfect/SKILL.md` 的同源 Codex 版本。Body 内容完全一致,只换了 frontmatter + 加了 Codex 用法说明。
...
- 用户说 "可以 X" / "可以 Y"(表示解冻之前冻结的设计约束)
- 用户列了一个 6-10 项的 priority 清单(信号:用户在想 scope)
- 之前讨论过 "{具体 feature 暂缓}" / "M2+ 路线" 之类的"暂缓但保留"项
...
| Braid | 1 个平台谜题 | 时间倒流 |
| [其他 {your project}] | (见 §9 case study) | (见 §9) |
...
| **4** | 实战踩坑 | {tech spike notes / onboarding / 性能 spike / bug fix checklist} | 最细的踩坑笔记,放在最后 |

> **为什么顺序固定**:Round 1 是"读者第一眼看到的",如果先改后面,Round 1 commit 时会反复 cross-reference 未提交的 doc,引用链会断。
>
4. **若存在归档目录**(如 `{归档目录名}/`),**必须**在 Round 1 同一 commit 加 {索引 README} 索引,说明"哪些是可恢复的 vs 哪些是永久 deprecated"。

**验证门**:每个 commit 类型检查 0 error + 引用链不断。

**Doc deliverable**:**{顶层入口 doc}** 更新(若有)。
...
2. **Scope 清单**(任务 / 房间 / 武器 / 角色 / 敌人 archetype / zone)— 按 §5.3 三分法分类
3. **冲突清单**(如 {美术 doc §forbidden list} "❌ X" vs Tier 2 粒子)

**每项标注**:
- 是否 ship 范围 / 是否数据冻结 / 是否 M2+ 路线(按 §5.3)
- 与现有规范的冲突点
- 解冻条件
...
**Doc deliverable**:**美术 doc 增** asset 清单 tier(按 §5.2)+ scope 清单(按 §5.3)+ 冲突清单。
...
>
> **plan doc 是 设计 doc 的核心章节**(§5.7 详述)。
...
**Doc deliverable**:**plan doc**(1 个,12 章节)+ 同步触发 **{设计 doc / 美术 doc / 代码 doc}** 三件套更新。
...
| 阶段 | 目标 | 子任务数 | 工期(参考) |
|------|------|---------|-------------|
| **P0** | {最小基础}(如:基础几何 / 占位渲染 / debug 视图) | 1-3 | 0.5-1 天 |
...
**MGP(最小可玩 + 闭环)**:P0-P7 ≈ {N} 天(参考)。
**SHIP-READY**:MGP + §5.6 完美定义全部勾选 ≈ {M} 天(参考)。
...
适用任何 "scope 砍到 1 + 打磨完美" 的项目。**plan doc 是 设计 doc 的核心章节**(§5.7)。
...
- **性能(机器)**:{target FPS} @ {target 分辨率} / 长时间稳定 / lastFrameTime < {frame budget} / 启动 ≤ 1s
...
|-----|---------|------|------|---------|------|
| **设计 doc**(GDD-level) | `gdd.md` 或 `design/` 目录 | 全员 + 玩家(README) | 5-30 页 | 机制改 / 范围改 / 整合规范改 | 一句话、机制、范围、视觉高层、整合、与 doc 间的引用。plan doc 是其核心章节 |
| **美术 doc**(Art Book) | `art-direction.md` 或 `art/` 目录 | 美术 + 程序(占位) | 10-50 页 | 调色板 / sprite 合同 / 资产清单 / forbidden list 改 | 调色板、sprite 合同、tile 风格、资产清单 tier(按 §5.2)、区域变体、forbidden list |
...
- **代码即文档**:数据契约文件(如 `{palette/data/contract}.ts` / `{资源表}.ts` / `{常量}.ts`)也算"代码 doc 的一部分",但 **不替代 prose**。
...
| 来源 | 用途 |
|------|------|
| `{项目 case study commit}` (YYYY-MM-DD) | 完整 case study(§9)— Part 1 deletion + Part 2 plan doc |
...
> **本节是 7_hotlineShanghai 项目的具体参考**(也是这个 skill 的来源项目)。如果你在另一个项目使用这个 skill,**不要直接复制本节的具体内容** — 数字 / 文件名 / 配色 / 引用链都来自 hotline shanghai 项目本身,你的项目会不同。
...
| 抽象 | hotline 实际 |
|------|-------------|
| **设计 doc** | `GDD.md`(v3 权威)/ `09-blindside-integration.md`(v3.1 BLINDSIDE 整合)/ `m1_intro_scene.md`(12 章节 plan)— 3 文件但逻辑上 1 个设计 doc |
