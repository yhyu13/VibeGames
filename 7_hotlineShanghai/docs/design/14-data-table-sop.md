...
| # | 表 | 文件 | 主权威 | 行数 / 字段 |
|---|----|------|--------|------------|
| 1 | 调色板 | `core/data/palette.ts` | TDD §4.4.8 | 12 PAL_* |
| 2 | 武器 | `core/data/weapons.ts` | TDD §4.4.1 | 8 WEAPON_TABLE |
...
### 3.3 面具 `masks.ts`(TDD §4.4.3 + 09 §6)
...
### 3.8 sprite `sprites.ts`(TDD §4.4.8 + 05-character)
...
| 场景 | 必改表 | 同步文档 |
|------|-------|---------|
| 加新面具 → 改 9 面具机制 | masks + missions(maskSpawns) + sfx(pickup_mask) | TDD §4.4.3 + 05 §3 + 03 |
| 加新武器 → 改 35 武器铺量 | weapons + missions(weaponSpawns) + sfx(fire_*) | TDD §4.4.1 + 05 + 03 |
| 加新敌人 archetype | enemies + missions(enemySpawns) + lights(`flashlight_patrol`)+ sfx(enemy 反馈) | TDD §4.4.4 + 09-§5 + 07 §5 |
| 加新房间 | missions + levels(md 蓝图)+ palette(zone)+ lights(decorativeLights) | TDD §4.4.5 + 08 + 02 §4 + 04 |
| 加新 RC 光源类型 | lights + enemies(archetype)+ palette | TDD §4.4.7 + 04 + 02 |
| 改调色板 | palette + sprites + lights + missions(floorPalette) | TDD §4.4.8 + 02 + 05 + 03 |
