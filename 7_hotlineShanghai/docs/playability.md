...
**拆中央油灯 → 灯灭后敌人半盲 → 近身安静击杀 → 走到绿色出口撤离结算。**
...
- **光 = 警觉开关**：灯亮敌人看见即 0.4s 电报 → 敌弹 OHK；灯灭敌人半盲（视锥 ×`DARK_VISION_MULT=0.5`）。
- **亮处击杀刷增援**：灯还亮时击杀（响亮）→ `triggerAlarm()` 刷增援；暗处安静击杀不触发。
- **阴影闪避**：玩家在暗处，敌弹 100% 落空（`SHADOW_SHOT_MISS=true`）。
