...
export interface LabPipelineConfig {
  ...
  twoLoop: boolean;          // demo 原版两轮 cascade（直射→间接）；false = 游戏旧实现的单轮变体
  mergeMode: 0 | 1 | 2;      // 0=demo 镜像映射 1=本像素双线性 2=射线终点双线性
  canonicalSpacing: boolean; // 1=canonical(cascade0 最细) 0=demo(反转)
}
...
export const DEFAULT_LAB_CONFIG: LabPipelineConfig = {
  ...
  ditherEnabled: false,
  jfaPasses: -1,
  twoLoop: true,             // demo 原版两轮（直射→间接）；false = 游戏旧实现的单轮变体
  mergeMode: 0,
...
export class RcLabPipeline {
  ...
  private directTarget: RenderTarget | null = null;   // 两轮模式：第一轮（直射）输出
  ...
  /**
   * Debug helper: run the cascade chain from the coarsest level down to
   * stopIndex (direct loop, mixFactor=0) and store the result into
   * radianceOut so readPixel('radiance', ...) can inspect a single level.
   * Call render() again afterward to restore the display state.
   */
  debugRenderCascade(stopIndex: number): { width: number; height: number } {
    const t = this.requireTarget('cascadeA');
  ...
  /** Debug helper: read one pixel from a JFA ping-pong target. */
  ...
  debugUniforms(): Array<[string, boolean]> {
    ...
    const p2 = this.programs.rcMerge;
    const out: Array<[string, boolean]> = [];
    for (const [name, loc] of p.uniforms) {
      out.push([name, loc !== null]);
    }
    for (const [name, loc] of p2.uniforms) {
      out.push([name, loc !== null]);
    }
    return out.sort((a, b) => a[0].localeCompare(b[0]));
  }

  /** 往 radianceOut 清一个已知色，验证 readTarget('radiance') 读的是否同一目标（调试用） */
  debugMarkRadiance(): number[] {
    const gl = this.gl;
  ...
  /** 检查 radianceOut 的 READ 绑定状态（调试用） */
  debugReadFbStatus(): { boundOk: boolean; statusHex: string; isCascadeA: boolean; isCascadeB: boolean } {
    ...
    const a = this.cascadeA;
    const b = this.cascadeB;
    return {
      boundOk: bound === t.framebuffer,
      statusHex: status.toString(16),
      isCascadeA: a !== null && a.framebuffer === t.framebuffer,
      isCascadeB: b !== null && b.framebuffer === t.framebuffer,
    };
  }

  /** 验证 readPixels 到底读 FBO 还是屏幕（调试用） */
  debugProbeReadback(): { readOk: boolean; drawBindingOk: boolean; bytes: number[] } {
    ...
    gl.bindFramebuffer(gl.READ_FRAMEBUFFER, null);
    return {
      readOk: readBinding === t.framebuffer,
      drawBindingOk: drawBinding === null,
      bytes: Array.from(bytes),
    };
  }

  /** 测试不同尺寸 readPixels 是否新鲜（调试用） */
  ...
  /**
   * 读回单个像素（RGBA8）。注意：本环境（ANGLE/SwiftShader）整缓冲 readPixels
   * 存在陈旧缓存 bug，断言一律用 1px 读取。
   */
  ...
  /** 调试视图：把阶段纹理直出到屏幕（随后 main 用 toDataURL 抓取） */
  ...
  private resize(width: number, height: number): void {
    ...
    this.cascadeA = this.recreateTarget(this.cascadeA, w, atlasH, gl.RGBA8, gl.RGBA, gl.UNSIGNED_BYTE, gl.LINEAR);
    this.cascadeB = this.recreateTarget(this.cascadeB, w, atlasH, gl.RGBA8, gl.RGBA, gl.UNSIGNED_BYTE, gl.LINEAR);
    this.intervalTarget = this.recreateTarget(this.intervalTarget, w, atlasH, gl.RGBA8, gl.RGBA, gl.UNSIGNED_BYTE, gl.LINEAR);
    this.directTarget = this.recreateTarget(this.directTarget, w, atlasH, gl.RGBA8, gl.RGBA, gl.UNSIGNED_BYTE, gl.LINEAR);
  ...
  private requireTarget(
    stage: LabReadStage | 'jfaA' | 'jfaB' | 'jfaOut' | 'cascadeA' | 'cascadeB' | 'interval' | 'radianceOut' | 'directTarget' | 'seed' | 'sdf' | 'final',
  ): RenderTarget {
    const t =
