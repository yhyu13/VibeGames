...
function holdme()
{
   if(!stopAll)
   {
      ...
      if(_parent[mst] != undefined)
      {
         ...
         this._sW = _parent[mst].shotw;
         this._sH = _parent[mst].shoth;
...
function fsme()
{
   if(!stopAll)
   {
      ...
      {
         _parent[mst][wpname].wpmodon = false;
      }
      else
      {
         _parent[mst][wpname].wpmodon = true;
      }
      if(_parent[mst] != undefined)
      {
         ...
         else
         {
            ...
            if(this._parent[this._name + "tx"] == undefined)
            {
               ...
               this._parent.attachMovie("tx_axe",this._name + "tx",this._parent.getNextHighestDepth(),{_sX:this._parent[mst]._sX,_sY:this._parent[mst]._sY,_sZ:this._parent[mst]._sZ,mst:this.mst});
               this._parent[this._name + "tx"].sz = this.sz;
            }
            if(this._parent[this._name + "tx2"] == undefined && this.wptx == "tx_axe2")
            {
               this.mz = true;
               this._parent[mst].GDdd = 5;
               this._parent.attachMovie("tx_axe",this._name + "tx2",this._parent.getNextHighestDepth(),{_sX:this._parent[mst]._sX,_sY:this._parent[mst]._sY,_sZ:this._parent[mst]._sZ,mst:this.mst});
