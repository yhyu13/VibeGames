...
> **2026-08-09 approved replacement:** the active intro is the replayable
> dense-voxel ATTACK showcase specified by
> `docs/2026-08-09-voxel-intro-physics-design.md`. The awakening ritual below
> is retained only as superseded history. The runtime is `main.tsx ->
> IntroEngine`: enter `W A W A`, watch fixed-X minions hop/squash, then a
> ballistic arrow opens a persistent dark crater with 20-64 pooled physical
> voxel fragments and localized warm impact light. The title appears over the
> crater; R/PLAY AGAIN restores the cached stage.
...
>
> **2026-08-09 recovery decision**:用户选择 `Intro-only shell`。当前构建刻意不接入
> 已损坏的 v0.1/v2.0 战斗混合层;完成觉醒后停在 `RITUAL COMPLETE` 展示卡。
> 战斗、HUD、命令语法均为后续恢复范围,不属于本轮开场验收。
...
眼睛;第 4 下所有眼睛点亮,军队觉醒,boss 咆哮,完成卡出现。
...
| 类别 | 内容 | 说明 |
|---|---|---|
| **ship 范围** | 1 个开场场景(觉醒仪式) | Intro-only shell,只做这一个场景 |
...
**关键决策 D1**:intro-only shell 不接入旧 FSM;`IntroDirector` 直接驱动五阶段
演出状态,完成后停在结果卡。
...
| ready | 4 次命中后 | 觉醒高潮(眼睛全亮 + chord + 观众欢呼 + boss 咆哮红光震屏)→ `RITUAL COMPLETE` | `commandResolve` / `audienceCheer` / `bossRoar` |
...
| P4 | UI 字幕/提示/跳过 | IntroScene.tsx 渲染 5 个 stage;App 门控完成卡 | 全程无 console error |
| P5 | 相机缓推 + 收尾 | title 逐字 / reveal 提示 / SKIP 按钮 | 手动播放 3 遍无 bug |
| P6 | 性能 + 稳定 | draw calls <= 11(硬 15);无泄漏 | 60 FPS 观察 |
| P7 | 端到端 playtest | 开机 -> 点击 -> 敲鼓 -> 觉醒 -> 完成卡 | 10 遍循环 |
...
浏览器手动(dev server 5183):开场黑场 -> 首次点击 -> 敲 4 次觉醒 ->
完成卡;同时验证 SKIP 快路径,全程 console 0 error。
...
| # | 决策 | 状态 | 决定者 |
|---|---|---|---|
| D1 | intro-only shell 不接入旧 FSM | 已解决 | 用户选择 `Intro-only shell` |
...
```
点击 黑场 ─ 1.2s 标题 ─ 4s 提示 ─ 6.5s 节拍 ─ ~10s 觉醒高潮 ─ 完成卡
                                   (敲得快 = 更早觉醒)
```
...
| 修改 | `src/engine/SceneManager.ts`(暗场/推进)、`src/engine/IntroEngine.ts`(场景/输入/音频接线)、`src/store.ts`(intro mirror)、`src/App.tsx`、`src/styles.css` |
| 文档同步 | `GDD.md` 搂6 开场、`TDD.md` 搂2 架构树、`README.md`、`CLAUDE.md` FSM 注记、`verification-report.md` |
| 归档 | 无(旧文档已在 Part 1 删除) |

## 12. 状态

- P0 鼓垫 + 暗场:✅ intro-only shell
- P1 开场轮廓:✅ 3-unit army + Moloch silhouette
- P2 交互节拍器:✅ 60 BPM / W-A-S-D / 4 hits
- P3 觉醒高潮:✅ eye pop + boss reveal + shake + audio
- P4 UI 字幕/提示/跳过:✅
- P5 相机缓推 + 收尾:✅ `RITUAL COMPLETE` card
- P6 性能 + 稳定:🟡 build green; formal FPS capture pending
- P7 端到端 playtest:🟡 browser smoke + skip path PASS; 10-loop human playtest pending
