...
### 6.1 Intro scene (觉醒仪式, plan: `docs/design/05-intro-scene-plan.md`)

- Boot: black + "CLICK TO START"; first click unlocks audio with one deep drum.
- Title / reveal: PATAPONG 3D + "YOU ARE THE DRUMMER" + 4 key cards.
- Awaken: 60 BPM metronome; any drum inside the beat counts (timing-only);
  each hit opens one army eye; the 4th hit awakens the army (eyes flare,
  chord, audience cheer) while Moloch roars (red flash + shake).
- After the awakening the classic menu (PLAY + command grammar + stats)
  appears. The intro lives inside the MENU phase and replays on every
  visit; SKIP or click fast-forwards.
